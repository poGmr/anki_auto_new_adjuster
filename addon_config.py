...
class AddonConfig:
    ...
    def _load(self):
        ...
        if os.path.exists(config_path):
            try:
                ...
                logger.error(f"Failed to decode JSON from {config_path}: {e}")
    ...
    def _init_decks_update(self):
        logger.debug("_init_decks_update")
    ...
    def _add_new_decks_to_add_on_config(self):
        logger.debug("_add_new_decks_to_add_on_config")
    ...
    def _update_decks_in_add_on_config(self):
        ...
        for deck in mw.col.decks.all_names_and_ids():
            ...
            if d_id in self.raw["decks"]:
                ...
                logger.warning(f"Deck ID {d_id} not found in the configuration.")

    def _remove_old_decks_from_add_on_config(self):
        logger.debug("_remove_old_decks_from_add_on_config")
    ...
    def get_global_state(self, key: str):
        logger.debug(f"get_global_state key {key}")
        if key in self.raw["global"]:
            return self.raw["global"][key]
        else:
            logger.error(f"get_global_state error")
            return None

    def set_global_state(self, key: str, value):
        logger.debug(f"set_global_state key {key} value {value}")
        if key in self.raw["global"]:
            self.raw["global"][key] = value
        else:
            logger.error(f"set_global_state error")
    ...
    def get_deck_state(self, did: str, key: str):
        if did in self.raw["decks"] and key in self.raw["decks"][did]:
            ...
            # logger.debug(f"get_deck_state did {did} key {key} value {value}")
            return value
        else:
            logger.error(f"get_deck_state did {did} key {key}")
            return None

    def set_deck_state(self, did: str, key: str, value: str | int | float):
        logger.debug(f"set_deck_state did {did} key {key} value {value}")
        valid_json_types = (str, int, float, bool, type(None), list, dict)
        if not isinstance(value, valid_json_types):
            logger.error(f"set_deck_state error: Invalid value type {type(value)} for key {key}")
            return
        if did in self.raw["decks"]:
            self.raw["decks"][did][key] = value
        else:
            logger.error(f"set_deck_state error")
    ...
    def get_duplicated_config_ids(self) -> list[str]:
        ...
        logger.debug(f"duplicated_config_ids {duplicated_config_ids}")

...
class DeckConfig:
    ...
    def set_new_count(self, new_count: int) -> None:
        if self.raw_data["new"]["perDay"] == new_count:
            logger.debug(f"[{self.name}] No change of value ({new_count}) is needed.")
            return
        if new_count < 0:
            logger.debug(f"[{self.name}] Input value ({new_count}) is less than zero.")
            return
        self.raw_data["new"]["perDay"] = new_count
        mw.col.decks.save(self.raw_data)
        logger.debug(f"[{self.name}] value {new_count} of new cards per day has been saved to the configuration.")
